...
\begin{abstract}
Learned world models distill environment dynamics into a neural substrate and act as differentiable surrogates for policy learning and planning. Most work to date trains one model per game on visually rich but mechanically uniform pixel observations. We instead train a single rule-conditioned world model across hundreds of mechanically diverse grid-based puzzle games, foregoing pixels for the full discrete game state. We propose to use a Neural Cellular Automaton (NCA) that mirrors a general-purpose symbolic game engine's repeated application of atomic local rewrites, cross-attending to slot embeddings produced by a jointly-trained autoencoder over symbolic game descriptions with the aim of enabling adaptation to unseen games at runtime. By scaling data in this regime we hope to shift from modeling a game to modeling an engine: a space of possible worlds organized by their dynamics.
