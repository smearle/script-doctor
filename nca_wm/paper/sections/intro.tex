...
The symbolic-AE component of contribution 3 is functional: the
joint encoder + slot-token decoder achieves teacher-forced
reconstruction with median per-token accuracy of $1.0$ and
$76/94$ perfectly reconstructed games across the 94-game preset
(\S\ref{sec:results-autoencoder}). Autoregressive sampling from the
empirical slot prior produces engine-loadable PuzzleScript at
$15/25$ random samples (60\%), and slot-space interpolation between
two training games hits $9/9$ engine-loadable points (100\%) along
the line. Inverse fitting from OOD transitions drives per-cell BCE
down 2--3 orders of magnitude on three held-out games and recovers
1-NN training-game neighbours that match the target's mechanics
(\S\ref{sec:results-inverse-fit}).
