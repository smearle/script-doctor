...
For each transition $(s_t, a_t, s_{t+1})$ in the training set we
encode the game's token sequence to a slot matrix
$z \in \mathbb{R}^{K \times d_{\text{slot}}}$ via the perceiver-style
encoder $\mathrm{Enc}_\phi$ and drive the rule-conditioned NCA from
$(s_t, a_t)$ conditioned on $z$ to produce $\widehat{s}_{t+1}$.
The training loss is
\begin{equation*}
  \mathcal{L} \;=\; \mathcal{L}_{\text{state}}(\widehat{s}_{t+1}, s_{t+1})
  \;+\; \alpha\,\mathcal{L}_{\text{win}} \, .
\end{equation*}
Default dimensions: $K=16$, $d_{\text{slot}}=64$, encoder
$d_{\text{model}}=64$ with two self-attention layers.
