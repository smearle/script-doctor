...
authored training set with $K{=}64$ synthetic levels at the game's
authored max-dim and evaluates on \emph{every} authored level (no L0
/ heldout split: all authored levels are out of distribution to the
training set). Recipe (validated post-1fa557d on single-game Sokoban
$\to$ Microban):
\texttt{--synthetic\_levels 64 --synthetic\_per\_game\_size
