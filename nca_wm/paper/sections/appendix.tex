...
authored training set with $K{=}128$ synthetic levels generated at the
...
synthetic small-grid training to bigger authored levels (validated
prior in \texttt{project\_synth\_recipe.md}: Sokoban $w{=}7 \to 0\%$
on 155 \textsc{Microban}/\textsc{Microban\_I} authored levels) while
size-down does not; (ii) using a single size keeps the per-size data
density at $K{=}128$, which empirically matters: an early multi-grid
variant ($K // n_{\text{sizes}}$ per size, $\sim 16$ levels per size
for Microban) plateaued at $\sim 8\%$ BFS heldout while the same recipe
at fixed $K{=}128$ on a single small size reached $0.60\%$ BFS, $0.02\%$
1-step. The earlier auto-bounding-box choice
(\texttt{--synthetic\_per\_game\_size}, which picks $\max W \times \max H$)
was also tried and failed at $\sim 9\%$ BFS — many authored levels are
much smaller than the bounding box and the synth distribution missed
them; we document the failure in
\S\ref{app:synth-only-failed-attempts}. Smallest-by-area (rather than
per-axis $\min W \times \min H$) is used so the chosen size is an
actually-existing authored level shape rather than a per-axis composite
(\textsc{Microban} per-axis min is $6{\times}6$ but no authored level is
$6{\times}6$; smallest-by-area picks $6{\times}7$, which appears in
three authored levels). Recipe (validated post-1fa557d on
\textsc{Microban}, bucket D, $30{,}000$ updates):
\texttt{--synthetic\_levels 128 --synthetic\_w <min\_W>
--synthetic\_h <min\_H> --synthetic\_fallback\_dynamics
--synthetic\_no\_a\_count\_max 5 --synthetic\_mode evolve
