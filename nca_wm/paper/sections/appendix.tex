...
\papertodo{\textbf{Synth-only $\to$ all-authored generalization
(headline goal of this experiment line).} Replace each cell's authored
training set with $K$ synthetic levels at the game's authored grid
size(s) and evaluate on \emph{every} authored level (no L0 / heldout
split --- all authored levels are out of distribution). Recipe:
\texttt{--synthetic\_per\_game\_size --synthetic\_fallback\_dynamics
--synthetic\_no\_a\_count\_max 5 --token\_decoder\_loss\_weight 0.1};
launcher: \texttt{nca\_wm/scripts/run\_per\_game\_arch\_grid.sh} (add
synth flags). Target: $\sim 0\%$ cell-error on Sokoban-class games,
non-trivial transfer on the rest. If pure synth fails on a game, fall
back to seeding from one or a handful of authored levels in addition
to synth --- this is a lesser form of within-game generalization but
still informative about whether the architecture can extrapolate
mechanics from few demonstrations. Prediction: the held-out errors
in Fig.~\ref{fig:per-game-arch-heatmap} fall sharply on every game,
and the F3 sharpening on \textsc{Travelling\_salesman} (same loss,
$40{+}$pp rollout split) either persists or widens.}
