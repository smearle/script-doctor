...
\paragraph{Corpus.}
Our corpus follows the PuzzleJax collection
structure~\cite{anonymous2026puzzlejax}: Gallery, Archive, and gists
correspond here to \texttt{Gallery}, \texttt{scraped\_games}, and
\texttt{scraped\_games\_increpare}. We also include a small set of
curated custom games.
...
spanning the dataset-size axis: $\textsc{scaling\_14}$ (14 curated
games for ablations), $\textsc{scaling\_gallery\_v2}$ (59 gallery
games),
$\textsc{scaling\_gallery\_v3}$ (94 games), and
$\textsc{scaling\_gallery\_v4}$ (199 games extending into the
scrape pool). For each game we run bounded A* from each authored
level (60 s wallclock and 100k iteration budget per level) and cache
the resulting unique transitions on disk under bit-packing, with a
per-game cap of 200k transitions. Held-out evaluation games are
drawn from the deduped pool and verified disjoint from the training
set both by name and by token-sequence hash.
...
\begin{itemize}
  ...
  \item \textbf{Multi-game} (\textsc{scaling\_14}): a fourteen-game
    ...
    $32{\times}24$.
  \item \textbf{Broad-corpus} ($\textsc{scaling\_gallery\_v3}$ and
    $\textsc{scaling\_gallery\_v4}$): 94 and 199 games respectively,
    all drawn from the deduped pool described above. Used for the
    multi-game scaling curve and for held-out-game evaluation. The
    held-out set is a 30-game stratified sample from the same pool,
    disjoint from both presets by name and by token-hash.
\end{itemize}
