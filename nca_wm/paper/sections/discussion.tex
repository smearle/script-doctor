...
\paragraph{From forward model to engine model.}
The abstract frames the project as a shift from modeling one game to
modeling a space of games organized by dynamics. The present results
support the first half of that claim: a rule-conditioned, iterated
...
non-iterative baselines. The remaining burden is to show that the
learned representation is genuinely engine-like: it should scale
smoothly with the number of games, generalize to held-out rule
systems, decode meaningful symbolic programs, and permit inverse
...
recovered slot near a mechanically related training game. The two
remaining gaps in the engine-modelling story are
(i) autoregressive sampling of new playable games (which currently
generates structurally valid but engine-load-failing PuzzleScript)
and (ii) breaking the multi-grid synthetic-data ceiling described
below.
...
``Near-perfect fidelity'' in the abstract refers to the
on-distribution regime: training on authored levels of human-written
games and evaluating on the same authored levels gives a median
