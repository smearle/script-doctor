...
across-game band.
Two model-independent references are shown.
The dotted black line is the identity (copy-last-state) baseline---per-corpus
state churn in-distribution (${\approx}2\%$) and $2.60\%$ on Heldout-26.
The brown dash-dot line is the no-rule baseline, which predicts the next state
by stepping the game with its rules removed: any player object moves in the
direction of the applied force unless a collision blocks it, and nothing else
moves.
This isolates plain force-driven movement from rule-driven behavior, and is a
much stronger reference than identity: ${\approx}0.4\%$ per corpus
in-distribution and $1.15\%$ on Heldout-26.
...
Out-of-distribution mean error falls toward the identity baseline as the
corpus grows but stays above the no-rule baseline, so on average the model
still trails a movement-only predictor on unseen games, even though its median
drops below the no-rule baseline at the largest corpus.}
