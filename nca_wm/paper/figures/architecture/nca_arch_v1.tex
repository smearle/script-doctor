
\documentclass[border=8pt, multi, tikz]{standalone}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{import}
\subimport{./}{init}
\usetikzlibrary{positioning}
\usetikzlibrary{calc}
\usetikzlibrary{fit}
\usetikzlibrary{3d}
\usetikzlibrary{decorations.pathreplacing}

% --- arch_overview color palette ---
\colorlet{EncColor}{blue!18}
\colorlet{EncBandColor}{blue!50}
\colorlet{SlotColor}{yellow!75}
\colorlet{NCAColor}{green!35}
\colorlet{NCABandColor}{green!70}
\colorlet{EmbedColor}{green!12}
\colorlet{ReadoutColor}{red!18}

\newcommand{\copymidarrow}{\tikz \draw[-Stealth,line width=0.8mm,draw={rgb:blue,4;red,1;green,1;black,3}] (-0.3,0) -- ++(0.3,0);}

\begin{document}
\begin{tikzpicture}
\tikzstyle{connection}=[ultra thick,every node/.style={sloped,allow upside down},draw=\edgecolor,opacity=0.7]
\tikzstyle{flowarrow}=[-Stealth, line width=0.5mm, opacity=0.85]
\tikzstyle{condarrow}=[-Stealth, line width=0.5mm, dashed,
    draw={rgb:blue,5;magenta,2;black,2}, opacity=0.9]
\tikzstyle{tflayer}=[draw, rounded corners=1.2pt, line width=0.3pt,
    minimum width=0.4cm, minimum height=2.4cm, inner sep=1pt]

% =============================================================
% LEFT: spec -> encoder -> latent (non-spatial)
% =============================================================

% --- input game spec ---
\node[draw, rounded corners=2pt, fill={gray!8},
      font=\footnotesize\ttfamily, align=left, inner sep=4pt,
      text width=3.6cm, anchor=east] (spec) at (-8.5,0)
{objects\\
=========\\
\textbf{Player} \textbf{Crate}\\
\textbf{Wall} \textbf{Target}\\
=========\\
rules\\
=========\\
{[}\textbf{>} Player | Crate{]}\\
~->{[}|\textbf{>}P. \textbf{>}Cr.{]}\\
=========\\
winconditions\\
=========\\
all Crate on Target};
\node[font=\small\itshape, anchor=south] at (spec.north) {game spec};

% --- Encoder block: 2 self-attn + 1 cross-attn ---
\node[tflayer, fill=EncColor]   (encS1) at (-6.8, 0) {};
\node[tflayer, fill=EncColor, right=0.08cm of encS1] (encS2) {};
\node[tflayer, fill=EncBandColor, right=0.08cm of encS2] (encX)  {};
\node[font=\scriptsize, rotate=90, inner sep=0pt] at (encS1) {self-attn};
\node[font=\scriptsize, rotate=90, inner sep=0pt] at (encS2) {self-attn};
\node[font=\scriptsize, rotate=90, inner sep=0pt, text=white!92!black]
    at (encX) {$K$-query x-attn};
\node[draw, rounded corners=2pt, line width=0.45pt, fit=(encS1)(encX),
      inner sep=3pt] (enc) {};
\node[font=\small\itshape, anchor=south] at (enc.north) {Encoder};
\node[font=\small, anchor=north] at (enc.south) {$\mathrm{Enc}_\phi$};

% --- Latent (slot/FiLM/VQ-agnostic) ---
\node[draw, rounded corners=1.5pt, fill=SlotColor, line width=0.4pt,
      minimum width=0.55cm, minimum height=2.6cm, anchor=center]
      (slots) at (-4.4, 0) {};
\node[font=\small\itshape, anchor=south] at (slots.north) {latent};
\node[font=\small, anchor=north] at (slots.south) {$z$};

% --- spec -> enc -> latent ---
\draw[flowarrow] (spec.east) -- (enc.west);
\draw[flowarrow] (enc.east)  -- (slots.west);

% =============================================================
% RIGHT: dynamics row (s_t/embed -> NCA body -> readout/s_{t+1})
% Box height=20, depth=20, scale=0.2 => visible y-z face is 4cm x 4cm,
% sized to the 3.6cm sokoban level images.
% =============================================================

% --- s_t (embed box, image projected on the east/front face) ---
\pic[shift={(-2.2,0,0)}] at (0,0,0)
    {Box={
        name=embed,
        caption= ,
        xlabel={{,}},
        zlabel={},
        fill=EmbedColor,
        height=20,
        width=2.5,
        depth=20
        }
    };
\node[canvas is zy plane at x=-1.7] (sin) at (0,0)
    {\includegraphics[width=3.6cm,height=3.6cm]{state_in.png}};
\path let \p1=(embed-south) in
    node[font=\large, anchor=north] at (\x1, -2.9cm) {$s_t$};

% --- NCA body: a single RightBandedBox with n_layers bands;
%     the whole body is iterated n_repeats times (loop arrow below).
%     Bands wider so per-band conditioning arrowheads don't overlap.
\pic[shift={(0.4,0,0)}] at (0,0,0)
    {RightBandedBox={
        name=nca,
        caption= ,
        xlabel={{,,}},
        zlabel={},
        fill=NCAColor,
        bandfill=NCABandColor,
        height=20,
        width={2.5,2.5,2.5},
        depth=20
        }
    };
\path let \p1=(nca-south) in
    node[font=\small\itshape, anchor=north] at (\x1-0.5cm, -2.9cm) {Convolutions};

% Brace + label for n_layers. Mirrored so the apex points UP. Label
% nudged to the right end of the brace so it does not collide with the
% cross-attn descent branches.
\path let \p1 = (nca-west), \p2 = (nca-east) in
    coordinate (braceLeft)  at (\x1+0.90cm, 2.90cm)
    coordinate (braceRight) at (\x2+0.90cm, 2.90cm);
\draw[decorate, decoration={brace, amplitude=6pt, raise=4pt, mirror}, line width=0.4pt]
    (braceRight) -- (braceLeft);
\node[font=\small\itshape, anchor=west]
    at ($(braceRight)+(-0.8cm,0.70cm)$) {$n_\text{layers}$};

% --- s_{t+1} (readout box, image projected on east/front face) ---
\pic[shift={(4.0,0,0)}] at (0,0,0)
    {Box={
        name=readout,
        caption= ,
        xlabel={{,}},
        zlabel={},
        fill=ReadoutColor,
        height=20,
        width=2.5,
        depth=20
        }
    };
\node[canvas is zy plane at x=4.5] (sout) at (0,0)
    {\includegraphics[width=3.6cm,height=3.6cm]{state_out.png}};
\path let \p1=(readout-south) in
    node[font=\large, anchor=north] at (\x1, -2.9cm) {$s_{t+1}$};

% --- forward connections through dynamics row ---
% Use -Stealth so the arrowhead lands AT the destination (nca-west /
% readout-west). The first arrow's head then coincides with the
% n_repeats loop's head at nca-west.
\draw[connection, -Stealth] (embed-east) -- (nca-west);
\draw[connection, -Stealth] (nca-east)   -- (readout-west);

% --- Loop arrow below the body (n_repeats). Tail at the right end of
%     the body (output of last band). Head ends just outside the left
%     edge of the body so the arrowhead clearly points INTO the first
%     band rather than disappearing into the box outline.
\path let \p1 = (nca-east), \p2 = (nca-west) in
    coordinate (loopOutR)  at (\x1+0.4cm, \y1)
    coordinate (loopOutRD) at (\x1+0.4cm, -4.0cm)
    coordinate (loopInLD)  at (\x2-1.1cm, -4.0cm)
    coordinate (loopInL)   at (\x2-1.1cm, \y2);
\draw[-Stealth, line width=0.7mm, opacity=0.32, rounded corners=10pt]
    (nca-east) -- (loopOutR) -- (loopOutRD) -- (loopInLD) -- (loopInL) -- (nca-west);
\node[font=\small\itshape, opacity=0.7]
    at ($(loopOutRD)!0.5!(loopInLD)+(0,-0.40)$) {$\times\, n_\text{repeats}$};

% =============================================================
% Conditioning: latent -> NCA body. Trunk routed above the figure;
% one arrowhead lands on each band's top so the conditioning is shown
% as applying to every layer individually.
% =============================================================
\tikzstyle{condtrunk}=[line width=0.5mm, dashed,
    draw={rgb:blue,5;magenta,2;black,2}, opacity=0.9]

\path let \p1 = (slots.north) in
    coordinate (slotsTop)  at (\x1, 2.05cm)
    coordinate (overslots) at (\x1, 4.5cm);

% Each band has scaled width 0.5cm. Band centers (world x):
%   band_i = nca-west.x + (0.25 + (i-1)*0.5)
\path let \p1 = (nca-west) in
    coordinate (band1Top) at (\x1+0.25cm, 2.10cm)
    coordinate (band2Top) at (\x1+0.75cm, 2.10cm)
    coordinate (band3Top) at (\x1+1.25cm, 2.10cm)
    coordinate (over1)    at (\x1+0.25cm, 4.5cm)
    coordinate (over2)    at (\x1+0.75cm, 4.5cm)
    coordinate (over3)    at (\x1+1.25cm, 4.5cm);

\draw[condtrunk, rounded corners=8pt]
    (slotsTop) -- (overslots) -- (over3);

\draw[condarrow, line width=0.5mm] (over1) -- (band1Top);
\draw[condarrow, line width=0.5mm] (over2) -- (band2Top);
\draw[condarrow, line width=0.5mm] (over3) -- (band3Top);

\node[font=\footnotesize, text=blue!55!black, anchor=south, align=center]
    at ($(overslots)!0.5!(over3)+(0,0.10cm)$)
    {conditioning};

\end{tikzpicture}
\end{document}
