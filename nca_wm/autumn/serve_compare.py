...
def snapshot():
    ...
    return {
        "models": list(S["runs"]), "current": S["cur"], "game": cfg["game"],
...
<div>
 model: <select id=model onchange="loadModel()"></select>
...
function render(s){
 ...
 if(sel.options.length!==s.models.length){sel.innerHTML='';
  s.models.forEach(m=>{let o=document.createElement('option');o.value=o.text=m;sel.add(o)});}
 sel.value=s.current;
