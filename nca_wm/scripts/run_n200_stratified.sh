#!/usr/bin/env bash
# n=200 stratified across n_rules 1..10 (20 games per bucket), cond+objperm.
# This is the rule-complexity-coverage pilot: same 200-game budget as the
# canonical sorted-mode n=200 baseline, but with training games drawn evenly
# from n_rules in [1..10] instead of all n_rules=1. Heldout-26 has median
# n_rules=6, so the stratified set actually covers the heldout complexity
# distribution.
#
# wait_for_gpu polls GPU 0 (held by n=800 cond+objperm) until it vacates,
# then launches. First training step will trigger A* on uncached
# high-n_rules games (~120 expected); training proper starts after that.
#
# Usage:
#   GPU=0 nohup nca_wm/scripts/run_n200_stratified.sh \
#       > /tmp/n200_stratified.log 2>&1 &
set -uo pipefail
...
SAVE_DIR=${SAVE_DIR:-nca_wm/logs/n_per_rule_200_cond_dp_strat10_objperm_val${VAL_FRAC}_s${SEED}}
...
echo "=== [$(date '+%F %T')] queueing n=200 stratified+objperm on GPU $GPU (save_dir=$SAVE_DIR) ==="
...
echo "=== [$(date '+%F %T')] launching n=200 stratified+objperm training ==="
...
echo "=== [$(date '+%F %T')] n=200 stratified+objperm done ==="
