...
def _aggregate_heldout(results_json_path: Path) -> dict:
    ...
    wins = sum(1 for a, b in zip(s1m, s1i) if a < b)
    return {
...
def write_latex(rows: list[dict], out_path: Path) -> None:
    """Compact 2-row table summarising the matched cond-vs-uncond comparison."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    # Identity baseline: same across rows; pull from first.
    if rows:
        ident_step1 = rows[0]["ood_step1_identity_mean"]
        ident_ar30 = rows[0]["ood_ar30_identity_mean"]
        body.append(
            "  "
            + " & ".join([
                "Identity baseline",
