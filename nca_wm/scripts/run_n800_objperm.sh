#!/usr/bin/env bash
# Continue the dedup_pool n_per_rule scaling curve to n=800 with the
# obj_permute_aug intervention baked in. obj_permute_aug at n=200 (s0)
# was the only intervention that beat the bare cond baseline on TF mean
# rollout error (2.47% vs 2.71%). If s1 confirms, we want this folded
# into all further scaling.
#
# wait_for_gpu is the same pattern as run_n_per_rule_dp_sweep.sh: poll
# nvidia-smi until GPU memory drops below the threshold (i.e. the
# in-flight n=400 cond run on GPU 0 has finished both training AND
# eval). Then launch n=800 cond + obj_permute on the same GPU.
#
# Usage:
#   GPU=0 nohup nca_wm/scripts/run_n800_objperm.sh \
#       > /tmp/n800_cond_objperm.log 2>&1 &
set -uo pipefail
...
SAVE_DIR=${SAVE_DIR:-nca_wm/logs/n_per_rule_800_cond_dp_objperm_val${VAL_FRAC}_s${SEED}}
...
echo "=== [$(date '+%F %T')] queueing n=800 cond+obj_permute on GPU $GPU (save_dir=$SAVE_DIR) ==="
...
echo "=== [$(date '+%F %T')] launching n=800 cond+obj_permute training ==="

CUDA_VISIBLE_DEVICES=$GPU PYTHONUNBUFFERED=1 \
    XLA_PYTHON_CLIENT_MEM_FRACTION=$XLA_PYTHON_CLIENT_MEM_FRACTION \
    XLA_FLAGS="--xla_gpu_enable_command_buffer=" \
    .venv/bin/python3 -u -m nca_wm.train \
    --n_per_rule_games 800 \
...
echo "=== [$(date '+%F %T')] n=800 cond+obj_permute done ==="
