#!/usr/bin/env bash
# In-distribution generalization sweep: for each n in {1, 2, 3}, train on
# the n simplest gallery games (sorted by rule count asc, then name asc)
# with `--val_frac 0.10` so 10% of each game's authored-level transitions
# are held out and never gradient-updated. Train and val loss/acc are
# logged to curves_step*.npz at every log_interval, ready for the
# train-vs-val curve plot.
#
# Defaults to GPU 0 on yalda; the queue polls until the GPU is free
# before launching each stage so it interleaves cleanly with other work.
#
# Usage:
#   nohup nca_wm/scripts/run_n_per_rule_sweep.sh \
#     > /tmp/n_per_rule_sweep.log 2>&1 &
set -uo pipefail
...
N_UPDATES=${N_UPDATES:-30000}   # smaller corpora: 30k is enough to see
                                # train/val divergence on overfit-prone runs.
...
#------------------------------------------------------------------
echo "=== [$(date '+%F %T')] n_per_rule sweep start (GPU $GPU, val_frac=$VAL_FRAC) ==="
wait_for_gpu

for n in 1 2 3; do
    stage "Train n_per_rule_games=$n cond s0 (val_frac=$VAL_FRAC)" \
        train_n_per_rule_cond "$n" 0 "nca_wm/logs/n_per_rule_${n}_cond_val${VAL_FRAC}_s0"
done

echo
echo "=== [$(date '+%F %T')] n_per_rule sweep done ==="
