...
LaTeX fragment per table:
  ...
  * architectures_compact.tex - Compact main-body table: BFS-AR per preset
  * architectures.tex      - Full appendix table (random/TF/BFS per preset)
...
ARCH_DISPLAY = {
    "nca_shared":  r"\textbf{NCA (shared)}",
    "nca_perstep": "NCA (per-step)",
...
def _table_compact_bfs(
    rows: list[dict],
    presets: list[tuple[str, str]],  # (column_header, games_key)
    sort_key_games: str,
    arch_order: list[str],
    out_path: Path,
) -> None:
    """Emit a compact BFS-only table: rows = arch, cols = preset.

    Cells are BFS-AR cell-error (mean ± std) per (arch, preset). Per-column
    BFS winner is bolded. Architectures are sorted by BFS-AR on
    `sort_key_games` ascending. The single params (M) column is taken from
    `sort_key_games` (Train-14 by default), which matches the multi-game
    headline; per-preset param drift is reported in the appendix table.
    """
    aggs_by_preset: dict[str, dict[str, dict]] = {}
    for _, games in presets:
        aggs_by_preset[games] = {a: _aggregate(rows, games, a) for a in arch_order}

    # Sort architectures by BFS-AR on the sort-key preset, ascending.
    def _sort_bfs(a: str) -> float:
        s = aggs_by_preset.get(sort_key_games, {}).get(a, {}).get("bfs_rollout_cellerr")
        return s[0] if s else float("inf")
    archs_sorted = sorted(
        [a for a in arch_order if any(aggs_by_preset[g].get(a) for _, g in presets)],
        key=_sort_bfs,
    )
    if not archs_sorted:
        print(f"[compact] no data; skipped {out_path.name}")
        return

    # Per-preset BFS winner (lowest mean) for bolding.
    best_per_preset: dict[str, str | None] = {}
    for _, games in presets:
        best, best_val = None, float("inf")
        for a in archs_sorted:
            s = aggs_by_preset[games].get(a, {}).get("bfs_rollout_cellerr")
            if s and s[0] < best_val:
                best, best_val = a, s[0]
        best_per_preset[games] = best

    # Per-preset min params for bolding (params drift slightly across presets;
    # we report params from sort_key_games but bold the global min there too).
    least_params = min(
        (aggs_by_preset[sort_key_games].get(a, {}).get("n_params") or float("inf"))
        for a in archs_sorted
    )

    body_lines = []
    for a in archs_sorted:
        params = aggs_by_preset[sort_key_games].get(a, {}).get("n_params")
        params_str = _fmt_params(params)
        if params is not None and params == least_params:
            params_str = f"\\textbf{{{params_str}}}"
        cells = [ARCH_DISPLAY.get(a, a), params_str]
        for _, games in presets:
            agg = aggs_by_preset[games].get(a, {})
            cells.append(_fmt_pct(
                agg.get("bfs_rollout_cellerr"),
                bold=(best_per_preset[games] == a),
            ))
        body_lines.append("  " + " & ".join(cells) + r" \\")

    col_spec = "l c" + " c" * len(presets)
    headers = ["Architecture", "params (M)"] + [h for h, _ in presets]
    table = (
        "% AUTOGENERATED by nca_wm/scripts/collate_baseline_tables.py — do not edit.\n"
    ...
    out_path.write_text(table)
    print(f"[compact] wrote {out_path}")
...
def main() -> None:
    ...
    _table_compact_bfs(
        ...
        arch_order=arch_order,
        out_path=OUT_DIR / "architectures_compact.tex",
    )
