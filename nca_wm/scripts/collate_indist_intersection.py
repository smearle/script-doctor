...
# Order matters for column layout: (preset, model) pairs across columns.
RUNS = [
...
HEADLINE = "random_tf"  # the per-game-rows table reports this regime
...
def write_table(rows_by_game: list[tuple[str, dict]], aggregates: dict, out_path: Path) -> None:
    """Emit per-game-rows table for the headline regime + aggregate footer.
    ...
    """
    cond_runs   = [(p, m, r) for p, m, r in RUNS if m == "cond"]
    uncond_runs = [(p, m, r) for p, m, r in RUNS if m == "uncond"]
    n_cond, n_uncond = len(cond_runs), len(uncond_runs)
    col_spec = "l " + " ".join(["c"] * (n_cond + n_uncond))
    ...
    headers = (
        ["Game"]
        + [f"{p} cond" for p, _, _ in cond_runs]
        + [f"{p} uncond" for p, _, _ in uncond_runs]
    )
    ...
    for game, row in rows_by_game:
        cells = [cell(row, r) for _, _, r in cond_runs] + [cell(row, r) for _, _, r in uncond_runs]
        # tt-wrap game name so underscores render
        safe_game = game.replace("_", r"\_")
        lines.append("  " + " & ".join([safe_game] + cells) + r" \\")
    lines.append("  \\midrule")
    for stat in ("mean", "median"):
        cells = (
            [_fmt_pct(aggregates[stat]["cond"].get(r)) for _, _, r in cond_runs]
            + [_fmt_pct(aggregates[stat]["uncond"].get(r)) for _, _, r in uncond_runs]
        )
...
def main() -> None:
    ...
    # Build per-game rows for the headline regime.
    rows_by_game: list[tuple[str, dict]] = []
    for game in intersection:
        row = {}
        for _, _, run_dir in RUNS:
            v = per_run[run_dir].get(game, {}).get(HEADLINE)
            if v is not None:
                row[run_dir] = v
    ...
    for preset, model, run_dir in RUNS:
        ...
        # Headline regime aggregates feed the table footer.
        head = run_summary["regimes"].get(HEADLINE)
        if head is not None:
    ...
    if missing:
        ...
        for r in missing:
            print(f"  {r}")
    print()
    print(f"Headline regime ({HEADLINE}) aggregates on the {len(intersection)}-game intersection:")
    print(f"  {'preset/model':<24s}  {'mean':>9s}  {'median':>9s}")
    for preset, model, run_dir in RUNS:
        head = summary[run_dir]["regimes"].get(HEADLINE)
        if head is None:
