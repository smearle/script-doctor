...
ALL_SEARCH_RESULTS_PATH = os.path.join('data', 'all_search_results.json')
...
def get_search_game_order(dataset: str = 'priority') -> list[str]:
    """Return games in the same order as plot_search_results.py: filtered to
    the given dataset, then sorted by mean pct_solved across all algos/depths
    (descending).  Display-formatted names are returned."""
    dataset_games = set(get_list_of_games_for_testing(dataset))

    if not os.path.exists(ALL_SEARCH_RESULTS_PATH):
        # Fallback: canonical dataset order, display-formatted
        return [format_game_name_for_display(g) for g in get_list_of_games_for_testing(dataset)]

    with open(ALL_SEARCH_RESULTS_PATH, 'r') as f:
        all_results = json.load(f)

    # Collect per-game pct_solved values across every algo / depth
    game_scores: dict[str, list[float]] = {}
    for _algo, depths in all_results.items():
        for _depth, games in depths.items():
            for game, stats in games.items():
                ...
                if pct is not None and not (isinstance(pct, float) and np.isnan(pct)):
                    game_scores.setdefault(game, []).append(float(pct))

    def _mean_score(game: str) -> float:
        vals = game_scores.get(game, [])
        return float(np.mean(vals)) if vals else -1.0

    ordered = sorted(game_scores.keys(), key=_mean_score, reverse=True)
    # Append any dataset games missing from search results
    remaining = [g for g in dataset_games if g not in ordered]
    ordered.extend(sorted(remaining))

    return [format_game_name_for_display(g) for g in ordered]
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='priority',
                        help="Dataset to use for game selection and ordering (default: priority).")
    args = parser.parse_args()

    current_script_path = os.path.dirname(os.path.abspath(__file__))
    results_dir = os.path.join(current_script_path, "llm_agent_results")
    output_dir = os.path.join(current_script_path, "llm_agent_results", "analysis")
    ...
    # Filter to games in the selected dataset
    dataset_games_display = set(format_game_name_for_display(g) for g in get_list_of_games_for_testing(args.dataset))
    ...
    output_path = os.path.join(output_dir, "llm_win_rate_heatmap.png")
    try:
        plt.savefig(output_path)
    ...
    output_path_steps = os.path.join(output_dir, "llm_steps_heatmap.png")
    try:
    ...
    if not llm_game_agg.empty:
        ...
        search_order = get_search_game_order(dataset=args.dataset)
        # Keep only games present in the data; append any extras not in search order
        present_games = set(llm_game_win_rates_plot.columns)
        heatmap_game_order = [g for g in search_order if g in present_games]
        extras = [g for g in llm_game_win_rates_plot.columns if g not in heatmap_game_order]
        heatmap_game_order.extend(sorted(extras))
        ...
        if not llm_game_win_rates_plot.empty:
            ...
            output_path_game_win = os.path.join(output_dir, "llm_vs_game_win_rate_heatmap.png")
            try:
            ...
            output_path_game_steps = os.path.join(output_dir, "llm_vs_game_steps_heatmap.png")
            try:
            ...
            plot_expanded_llm_heatmap(
                ...
                output_path=os.path.join(output_dir, "llm_vs_game_win_rate_expanded_heatmap.png"),
                cmap='RdYlGn',
            ...
            plot_expanded_llm_heatmap(
                ...
                output_path=os.path.join(output_dir, "llm_vs_game_steps_expanded_heatmap.png"),
                cmap='Blues',
    ...
    if not df.empty:
        ...
        for game_name in games:
            ...
            output_path_game_level_win = os.path.join(output_dir, f"game_{safe_game_name}_llm_vs_level_win_rate_heatmap.png")
            try:
            ...
            output_path_game_level_steps = os.path.join(output_dir, f"game_{safe_game_name}_llm_vs_level_steps_heatmap.png")
            try:
