...
from marl.env import LogEnvState, MultiAgentEnv, State
from marl.model import ScannedRNN
from spaces import Box as BoxGymnax, Discrete as DiscreteGymnax
from marl.spaces import Discrete, Box, MultiDiscrete
...
class MALogWrapper(JaxMARLWrapper):
    ...
    @partial(jax.jit, static_argnums=(0,))
    def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, PSState]:
        obs, env_state = self._env.reset(key)
        env_state: PSState
        state = LogEnvState(
            env_state,
    ...
    def step(
        ...
        info["returned_episode_returns"] = state.returned_episode_returns
        info["returned_episode_lengths"] = state.returned_episode_lengths
        info["returned_episode"] = jnp.full((self._N_AGENTS,), ep_done)
        return obs, state, reward, done, info


class MultiAgentWrapper(JaxMARLWrapper):
    def __init__(self, env, env_params: PSParams):
        super().__init__(env)
    ...
    def get_avail_actions(self, state: PSState):
        return {
